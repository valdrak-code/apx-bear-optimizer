...
function getInputs() {
  return {
    ...
    includeLeader: els.includeLeader.checked,
...
function buildBestLeaderMarch(leaderSize, available, joinerRatio = 0) {
  ...
  if (joinerRatio >= ARCHER_IDEAL && advanced.total > 0 && advanced.fillRatio >= 0.95 && advanced.ratio >= LEADER_ARCHER_ADVANCED - 0.02) {
    return advanced;
  }
  return standard;
}
function buildJoinerComp(size, joiners, available) {
  const maxInf = floorStep(available.infantry / joiners);
  const maxCav = floorStep(available.cavalry / joiners);
  const maxArch = floorStep(available.archers / joiners);
  const infSoft = Math.min(floorStep(size * INFANTRY_SOFT_CAP), maxInf);
  let infantry = infSoft;
  let archers = Math.min(floorStep(size * JOINER_ARCHER_ELITE_CEILING), maxArch, Math.max(0, size - infantry));
  let gap = size - infantry - archers;
  let cavalry = 0;
  if (gap > 0) {
    cavalry = Math.min(floorStep(gap), maxCav, archers);
    gap -= cavalry;
  }
  if (gap > 0) {
    const addArch = Math.min(floorStep(gap), Math.max(0, maxArch - archers));
    archers += addArch;
    gap -= addArch;
  }
  if (gap > 0) {
...
function generateCandidates(input, forceJoiners = null) {
  ...
  for (let joiners = min; joiners <= max; joiners++) {
    for (let size = floorStep(input.cap); size >= STEP; size -= STEP) {
      ...
      if (!comp) continue;
      let leader = null;
      if (input.includeLeader) {
        const remaining = subtractTroops(input.troops, comp, joiners);
        leader = buildBestLeaderMarch(input.leaderSize, remaining, comp.ratio);
        if (input.strategy === 'balanced' && input.troops.archers >= STEP && leader.total > 0 && leader.archers < STEP) continue;
      }
      const candidate = {
        joiners,
        size,
...
function strategyLabel(value) {
  if (value === 'joiners') return 'Prioritize Joiners';
...
function buildReport(result, input) {
  ...
  if (result.comp.cavalry <= result.comp.archers) report.push({ type: 'good', text: '🐎 Cavalry was used as filler without exceeding Archers.' });
  ...
  if (input.includeLeader && result.leader) {
    ...
    else report.push({ type: 'warn', text: '👑 Rally Leader Formation is limited by remaining troops.' });
  }
  return report;
}
function buildCoach(result, input) {
  ...
  if (input.includeLeader && input.strategy === 'leader' && result.joiners < input.maxMarches) return 'Leader Priority reduced joiner count to preserve your Rally Leader march. Switch to Joiner Priority when you are not launching rallies.';
  if (input.includeLeader && result.leader && result.leader.mode === 'advanced') return 'Advanced Rally Leader milestone achieved while preserving 70%+ Archer joiners. Maintain Archer growth to keep this setup viable.';
...
function buildInsights(result, maxSetup, input, allCandidates) {
  ...
  if (input.includeLeader && result.leader && input.leaderSize > 0) {
...
function renderMaxSetup(setup, recommendation, input) {
  const showLeader = input && input.includeLeader;
...
function renderDeveloperMode(allCandidates, recommended, maxMarchSetup, input, elapsedMs) {
  ...
  summary.textContent = `UI ${VERSION} | Engine ${ENGINE_VERSION} | Build ${BUILD_DATE} | Strategy ${strategyLabel(input.strategy)} | Candidates ${allCandidates.length} | Rejected/Below Floor ${rejectedCount} | Winner Score ${recommended ? recommended.score : 'N/A'} | Calc ${elapsedMs.toFixed(2)} ms`;
...
function render() {
  ...
  els.leaderSettings.style.display = input.includeLeader ? 'block' : 'none';
  els.leaderBreakdownCard.style.display = input.includeLeader ? 'block' : 'none';
  ...
  els.recSummary.textContent = `${statusRatingText(recommended.comp.ratio)} Formation selected using the ${strategyLabel(input.strategy)} strategy. Score: ${recommended.score}/100.`;
  ...
  if (input.includeLeader && recommended.leader) {
...
function buildCopyRecommendationText() {
  ...
  const grade = gradeFromScore(recommended.score);
  const leaderLines = input.includeLeader && recommended.leader ? [
    '',
  ...
  return [
    ...
    `Strategy: ${strategyLabel(input.strategy)}`,
    '',
...
for (const key of ['infantry', 'cavalry', 'archers', 'maxMarches', 'allianceCap', 'includeLeader', 'leaderSize', 'strategy']) {
