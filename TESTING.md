# APX Tools v2.2.0 Testing

Recommended checks after deploy:

- Open apxtools.org on desktop and mobile.
- Confirm navigation: Optimizer, Guide, About, Changelog.
- Run default values and press LET'S GOOOO.
- Test Rally Leader ON with each strategy.
- Confirm Copy Recommendation works.
- Confirm advanced Rally Leader detection does not weaken joiners below 70% Archers.
- Confirm APX Coach does not recommend reducing marches just to chase 100% Archers.
